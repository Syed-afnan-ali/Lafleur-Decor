...
// Get user info
$stmt = $conn->prepare("SELECT username, email FROM user_auth WHERE id = ?");
...
<head>
  ...
  <style>
    ...
    .box { background: white; padding: 20px; border-radius: 8px; width: 1000px; margin: auto; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
    input, textarea, button { width: 100%; padding: 10px; margin: 10px 0; }
    h2 { margin-top: 0; }
  </style>
</head>
<body>
<div class="hight" style="height: 38rem;">

    <div class="box" style="position: relative; top: 5rem;">
        <h2>🛒 Checkout</h2>
